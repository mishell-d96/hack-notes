...
# 3. Retrieve all hosts with smb-signing disabled
...
* [ ] **2.3 SMB - NTLM theft via writable share**
...
* [ ] **2.4 SMB - relay attack (responder + ntlmrelayx)**
...
* [ ] **2.5 SMB - relay attack  (coercion & ntlmrelayx)**
...
* [ ] **2.6 LDAP - Anonymous LDAP enumeration**&#x20;
...
* [ ] **2.7 RPC - Anonymous RPC enumeration**&#x20;
...
* [ ] **2.8 KERBEROS - Kerberos username enumeration**&#x20;
...
* [ ] **2.9 DNS - IPv6 takeover (Mitm6 + ntlmrelayx)**
...
* [ ] **3.2. SMB - password spraying (nxc, common passwords)**&#x20;
...
* [ ] **3.3. SMB - Password spraying (nxc, cewl, keywords & seasons)**&#x20;
...
* [ ] **4.1 bloodhound-python**

```bash
# 1. Bloodhound ingestor bloodhound-python (bloodhound-ce)
bloodhound-python -v -u '{{USERNAME}}' -p '{{PASSWORD}}' -d '{{DOMAIN}}'.'{{ROOTDNS}}' -dc '{{HOSTNAME}}'.'{{DOMAIN}}'.'{{ROOTDNS}}' -ns {{NAMESERVER}} -c All

# 2. Bloodhound ingestor nxc (bloodhound-ce)
...
* [ ] **4.5 nxc - quick-win vulnerability checks**&#x20;

```bash
# 1. check for multiple vulnerabilities in the AD environment (use latest version of nxc)
nxc smb {{RHOST}} -u '{{USERNAME}}' -p '{{PASSWORD}}' -M nopac -M enum_cve -M zerologon -M printnightmare -M smbghost -M ms17-010 -M coerce_plus
```

* [ ] **4.6 nxc & smbclient - authenticated share enumeration**&#x20;
...
# 3. check if you can impersonate a user (CHECK THIS FOR ALL AD USERS YOU HAVE!)
...
**4.9 adidnsdump - dns enumeration**
...
**5.3 - badsuccessor (**[**https://github.com/Akamai/BADSUCCESSOR**](https://github.com/Akamai/BADSUCCESSOR)**)**

```bash
# 1. (Windows server 2025!) utilize nxc ldap to check for badsuccessor privileges (checkout checkpoint.htb writeup)
nxc ldap {{RHOST}} -u '{{USERNAME}}' -p '{{PASSWORD}}' -M badsuccessor
```
